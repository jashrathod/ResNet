...
class ResNet(nn.Module):
    def __init__(self, block, in_planes, num_layers, num_blocks, kernel, skip_kernel, model_name, num_classes=10, bias=True):
        ...
        self.linear = nn.Sequential(
            nn.Linear(finalshape, finalshape),
            nn.Linear(finalshape, num_classes)
        )
...
model = ResNet(BasicBlock, 36, 4, [4, 4, 4, 2], kernel=(3, 1), skip_kernel=(
...
def main(model, data_augmentation=['trivial_aug'], epochs=100, optim="adadelta", batch_size=128, print_every=10, model_name='resnet'):
    ...
    print(
        f"Total Epochs : {EPOCHS} | Optimizer : {optim} | Learning Rate : {learningRate} | Batch Size : {batch_size}")
...
model_name = 'tra_hf_rc_ep200_optAdadelta_bs512'
...
main(model, data_augmentation=['trivial_aug', 'horizontal_flip', 'random_crop'],
     epochs=200, optim='adadelta', batch_size=512, model_name=model_name)
